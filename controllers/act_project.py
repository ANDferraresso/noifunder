...
#
class Act_Project(ctr.Controller):
    #
    ...
    def list_all(self, start_response):
        template_name = "act_project/list"
        ...
        # Filtri
        options = {
            "status": self.tmpl_vars['lc']['act_projectTab']['T']['opts']['status'],
        }
        conds, filter_arr, filters_js, self.tmpl_vars['lc']['filterHTML'] = self.paginator.manage_filter(
            "sqlite", self.act_projectMd, 
            ext_refs, quick_list, conds, options,
            self.tmpl_vars['cr']['links']['act_project_list'] + "/1" + "?", 
            "act_project_filter_btn", 4
        )
        self.tmpl_vars['cr']['js'] += filters_js
        self.tmpl_vars['lc']['list_qs'] = self.manage_list_query_string(order_by, order_prog, filter_arr)
        
        self.tmpl_vars['lc']['act_projectTab']['filtered'] = False
        if len(filter_arr) > 0:
            self.tmpl_vars['lc']['act_projectTab']['filtered'] = True
        order = []
        self.tmpl_vars['lc']['act_projectTab']['rowsNum'] = self.act_projectMd.select(True, ext_refs, quick_list,
            conds, [], None, 0, {})['data'][0]
        if self.tmpl_vars['lc']['act_projectTab']['rowsNum'] < 1:
            if par_page != 1:
            ...
            self.tmpl_vars['lc']['pagHTML'] = self.paginator.pag_html(
                self.tmpl_vars['lc']['act_projectTab']['rowsNum'], ROWS_PER_PAGE, par_page, '', pag_pars, 'U')
            if par_page < 1 or par_page > self.paginator.pages_num:
                ...
                self.tmpl_vars['lc']['act_projectTab']['data'] = self.act_projectMd.select(False,
                    ext_refs, quick_list, conds, order, limit, offset, {})['data']
                if len(self.tmpl_vars['lc']['act_projectTab']['data']) > 0:
                    self.tmpl_vars['lc']['act_projectTab']['hasData'] = True
                    self._manage_act_project()
    ...
    def create(self, start_response):
        template_name = "act_project/create"

        ext_refs = True
        quick_list = ["code", "title", "start_date", "deadline", "memo"]
        not_required = []
        prefix = ''
        self.form.import_table(
            self.act_projectMd.get_table_def(), self.tmpl_vars['lc']['act_projectTab']['T'],
        ...
        if self.tmpl_vars['env']['METHOD'] == 'GET':
            ...
            use_defaults = True
            after_submit = False
            self.tmpl_vars['lc']['act_projectTab']['renderedForm'], js = self.render.render_form(
                self.form.form_def,
                self.tmpl_vars['cr']['T'], self.tmpl_vars['lc']['act_projectTab']['T'], 
            ...
            if key in f_values and not key in wrong_fields and not key in self.form.form_def['dontValidate']:
                if not self.check_option(f_values[key], self.form.form_def['ui'][key]['options']):
                    ...
                    self.tmpl_vars['lc']['act_projectTab']['T']['msg'][key] += " " + self.tmpl_vars['cr']['T']['msg_ErrExtKey']

            # Verifica clausola unique
            wrong_fields = self.verify_uniques("C", None,
                self.act_projectMd, self.tmpl_vars['lc']['act_projectTab'],
                f_values, {},
                wrong_fields)
            ...
            if len(wrong_fields) == 0:
                ...
                location = self.tmpl_vars['cr']['links']['act_project_list'] + "/1"
                res = self.act_projectMd.insert(ps)
                self.close_controller()
                if res['err'] == True:
                    ...
                    location = self.tmpl_vars['cr']['links']['act_project_read'] + "/" + str(res['data'][0])
                    self.close_controller()
    ...
    def read(self, start_response):
        template_name = "act_project/read"
        
        ext_refs = False
        quick_list = []
        conds = [
            ['W', 'AND', self.act_projectMd.get_table_name(), 'rid', '=', int(self.ctx['params']['rid']), '']
        ]
        order = []
        res =  self.act_projectMd.select(False, ext_refs, quick_list, conds, order, 1, 0, {})
        print(res)
        self.tmpl_vars['lc']['act_projectTab']['data'] = self.act_projectMd.select(False,
        ...
        # Carica campagne del progetto
        cam_quick_list = ["rid", "code", "title", "status", "start_date", "deadline"]
        cam_conds = [
            ['W', 'AND', self.act_campaignMd.get_table_name(), 'project_rid', '=', int(self.ctx['params']['rid']), '']
        ]
        cam_order = [["code", "ASC"]]


        res = self.tmpl_vars['lc']['act_campaignTab']['data'] = self.act_campaignMd.select(False,
            False, cam_quick_list, cam_conds, cam_order, None, 0, {})
        print(res)



        self.tmpl_vars['lc']['act_campaignTab']['data'] = self.act_campaignMd.select(False,
            False, cam_quick_list, cam_conds, cam_order, None, 0, {})['data']
        if len(self.tmpl_vars['lc']['act_campaignTab']['data']) > 0:
            self.tmpl_vars['lc']['act_campaignTab']['hasData'] = True
            self._manage_act_campaign()

        # Project
        self.tmpl_vars['lc']['act_projectTab']['rendered'] = ""
        self.tmpl_vars['lc']['act_projectTab']['hasData'] = True
        f_key_links = {}
        fields_to_html_escape = []
        self._manage_act_project()
        self.tmpl_vars['lc']['act_projectTab']['rendered'] = self.render.render_record(
            self.act_projectMd.get_table_columns_keys(),
            self.act_projectMd.get_table_fKeys(),
            self.tmpl_vars['lc']['act_projectTab']['T'], 
            self.tmpl_vars['lc']['act_projectTab']['_data'],
    ...
    def update(self, start_response):
        template_name = "act_project/update"
    ...
    def delete(self, start_response):
        template_name = "act_project/delete"
        
        self.close_controller()
        return self.tmpl.render_page(start_response, template_name, self.tmpl_vars, "", [], self.cookies)

    #
    def _manage_act_project(self):
        self.tmpl_vars['lc']['act_projectTab']['_data'] = []
        if len(self.tmpl_vars['lc']['act_projectTab']['data']) > 0:
            keys = list(self.tmpl_vars['lc']['act_projectTab']['data'][0].keys())
            c = 0
            for row in self.tmpl_vars['lc']['act_projectTab']['data']:
                self.tmpl_vars['lc']['act_projectTab']['_data'].append({})
                for key in keys:
                    if key == "status":
                        self.tmpl_vars['lc']['act_projectTab']['_data'][c][key] = self.get_option(
                            str(row[key]), self.tmpl_vars['lc']['act_projectTab']['T']['opts'][key])
                    else:
                        self.tmpl_vars['lc']['act_projectTab']['_data'][c][key] = row[key]
                c += 1
